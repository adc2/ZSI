function [idx,val,orient,cc]=find_source_orient(GLF3D,nTarget,exponent)
...
cmax=max(cost(:));
for iLoc=1:nlocations
    cost(iLoc,find(dd(iBest(iLoc),:)>1))=cmax; % zap orientations opposite to best (cost pattern is bimodal on sphere)
    [~,idxBest]=mink(cost(iLoc,:),4);
    oBest=oGrid(idxBest,:);
    cBest=cost(iLoc,idxBest);

    % interpolate as center of gravity
